...
| 项目规模 4k+ MoonBit LOC | 当前 MoonBit 源码约 `7811` 行 |
...
- enum parser/codegen/runtime/JSON，含 schema-aware JSON enum-name mapping；
...
- protobuf-style JSON writer/parser，包含 protobuf JSON enum-name schema mapping、bytes URL-safe/unpadded base64 输入、`null`-as-absent 解析语义与 lowerCamelCase 字段名输入/输出 helper；
