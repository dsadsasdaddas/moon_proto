...
- official differential harness：`python3 scripts/moon_proto_official_diff.py --report generated/official_diff_report.md --junit-out generated/official_diff_report.xml`，并在 CI 中克隆 `moonbitlang/protoc-gen-mbt` 校验官方源码/文档契约；也支持 `--official-generated-dir tests/differential/official_generated_fixture` 对预生成官方 `.mbt` 输出执行 generated-output contract 检查；支持 `--official-plugin-bin` 对已安装 `protoc-gen-mbt` 执行 live-generator smoke path；
...
- conformance-lite evidence report：`python3 scripts/moon_proto_conformance.py --report generated/conformance_lite_report.md --json-out generated/conformance_lite.json --junit-out generated/conformance_lite.xml`，把 oracle-backed fixtures 汇总为 Markdown/JSON/JUnit 验收证据，并内置 expected-fail mutation self-checks 验证报告能抓住篡改 fixture、缺失 artifact 和 JSON 证据丢失；同时输出 semantic-axis coverage gates，要求 binary/json/map/oneof/numeric/float/wire-decode/unknown-field/negative 等轴达到最低覆盖数。
...
- MoonBit tests: `43/43 passed`；
- `moon test --target all`: wasm、wasm-gc、js、native 全通过；
- generated-code compile check 通过；
- Schema Doctor、AI verify report、official differential report、installed-plugin official generator smoke report、conformance-lite evidence report（含 upstream-style wire-decode edge vectors、expected-fail mutation self-checks 与 coverage gates）、descriptor verify/compat/registry/policy report and JUnit XML 与 CI 官方源码契约检查通过；
...
- 更大规模的 upstream protobuf conformance suite 子集和官方 MoonBit protobuf differential tests；
