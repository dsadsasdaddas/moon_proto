...
- Python/Go 官方 protobuf oracle fixtures，用于验证 scalar/repeated/map/oneof/32-bit numeric/float/double golden bytes 与 JSON 兼容性。
...
│   └── file-based .proto to .mbt generator, Schema Doctor, compatibility checks and AI verify reports
├── tests/oracle/
...
- float/double NaN、Infinity 等特殊 JSON 字符串语义；
- conformance suite 子集接入；
- schema 兼容性对比：`compat old.proto new.proto` 风格的破坏性变更检查；
