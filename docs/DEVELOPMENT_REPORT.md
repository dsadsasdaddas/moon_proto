...
- 重点提供动态 schema lab、Schema Doctor、schema validation、compatibility fixtures、protobuf JSON mapping 检查、AI verify 报告、generated-code compile check 和 AI 生成代码验证；
...
- `.proto` lexer/parser：syntax、package、message、enum、optional/repeated、map、oneof、标量字段；
- schema validation diagnostics 与 Schema Doctor CLI：字段号范围、重复字段名/号、proto3 enum 首值、顶层命名冲突、map 约束，输出稳定诊断路径；
...
- file-based lab CLI：`python3 scripts/moon_proto_lab.py doctor/inspect/verify schema.proto --report report.md`；
...
├── scripts/moon_proto_gen.py / scripts/moon_proto_lab.py
│   └── file-based .proto to .mbt generator, Schema Doctor and AI verify reports
├── tests/oracle/
...
- Schema Doctor 与 AI verify report 检查通过；
...
- 与 `moonbitlang/protobuf` / `protoc-gen-mbt` 建立 adapter 或 differential tests；
...
- schema 兼容性对比：`doctor old.proto new.proto` 风格的破坏性变更检查；
