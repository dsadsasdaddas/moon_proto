...
- Schema Doctor、AI verify report、official differential report、conformance-lite evidence report（含 expected-fail mutation self-checks 与 coverage gates）、descriptor verify/compat/registry/policy report and JUnit XML 与 CI 官方源码契约检查通过；
...
- 在官方仓库依赖图可解析且安装 `protoc` 的环境中执行 live official generator differential tests；
