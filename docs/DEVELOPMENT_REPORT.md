...
Moon Proto Lab 是面向 MoonBit protobuf 生态的动态 schema、兼容性测试与 AI 代码验证工具链。项目目标不是重复已有官方 protobuf runtime，而是提供一套可复用、可测试、可持续维护的验证基础设施：解析 `.proto` schema，构建 MoonBit descriptor，执行动态 message 二进制/JSON roundtrip，生成 MoonBit 代码实验产物，并使用 Python/Go 官方 protobuf 作为 cross-language oracle。
...
- 重点提供动态 schema lab、schema validation、compatibility fixtures、protobuf JSON mapping 检查、generated-code compile check 和 AI 生成代码验证；
...
- schema validation diagnostics：字段号范围、重复字段名/号、proto3 enum 首值、顶层命名冲突、map 约束；
...
│   └── file-based .proto to .mbt generator wrapper
├── tests/oracle/
