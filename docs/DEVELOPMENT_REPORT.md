...
- enum parser/codegen/runtime/JSON 数值映射与 schema-aware JSON enum-name mapping；
...
- protobuf-style JSON writer/parser，覆盖 schema-aware enum-name mapping、bytes base64（含 URL-safe alphabet 与省略 final padding 输入）、64-bit integer string form、repeated/map/nested/oneof、`null`-as-absent 解析语义与 lowerCamelCase 字段名输入/输出 helper；
