...
- `.proto` lexer/parser：syntax、点分 package、import、option、真实 reserved number/name descriptor、extensions、message、enum、nested message/enum lifting、optional/repeated、map、oneof、字段/枚举值 options、signed enum value / enum reserved range、empty statement、标量字段、点分 named type、block comment 与忽略式 service/rpc block 容错；
